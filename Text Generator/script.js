...
let buttontElem = $.querySelector('.btn')
let resultElem = $.querySelector('#result')

function textGen(e){
     e.preventDefault();
     let inputValue = inputElem.value
     let randomNum = Math.floor(Math.random()*dummyText.length)
     if(inputValue<0 || inputValue>9|| inputValue === ''||isNaN(inputValue)){
        resultElem.innerHTML = dummyText[randomNum]
     }else{
         let sliced = dummyText.slice(0,inputValue)
         let sliceP = sliced.map(function(item){
             return '<p>'+item+'</p>'
         }).join('')
         resultElem.innerHTML=sliceP
     }
}
buttontElem.addEventListener('click',textGen)